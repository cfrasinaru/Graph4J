setwd("d:/java/Graph")
paperDir <- "d:/scoala/work/graph4j/"
...
create_eps("CompleteGraphDemo1", ylim1=c(0,2000)) #all
...
#create_eps("IterateEdgesDemo")
#create_eps("ContainsEdgeDemo")

setEPS()
postscript(paste(paperDir, "eps/IterateContainsDemo.eps",sep=""), width=7, height=4)
par(mfrow=c(1,2))
plot_results("IterateEdgesDemo", "time")
plot_results("ContainsEdgeDemo", "time")
dev.off()
...
setEPS()
postscript(paste(paperDir, "eps/RemoveEdgesNodes.eps",sep=""), width=7, height=4)
par(mfrow=c(1,2))
...
setEPS()
postscript(paste(paperDir, "eps/DijkstraDemo.eps",sep=""), width=7, height=4)
par(mfrow=c(1,2))
...
setEPS()
postscript(paste(paperDir, "eps/MSTDemo.eps",sep=""), width=7, height=4)
par(mfrow=c(1,2))
...
#setEPS()
#postscript(paste(paperDir, "eps/XYDemo.eps",sep=""), width=7, height=4)
#par(mfrow=c(1,2))
