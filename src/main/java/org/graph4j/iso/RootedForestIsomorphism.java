...
public class RootedForestIsomorphism implements Isomorphism {
     ...
     * @throws NullPointerException if {@code forest1}, {@code forest2}, {@code roots1} or {@code roots2} is {@code null}
     * @throws IllegalArgumentException if {@code forest1} or {@code forest2} is not undirected
     * @throws IllegalArgumentException if {@code forest1} or {@code forest2} is not simple(no self loops or multiple edges)
     ...
     * @throws IllegalArgumentException if {@code roots1} or {@code roots2} are not in the forest
     */
     ...
     *     Does the following:
     *     <ul>
     *         <li>If the forests have a single root, then it checks if the two trees are isomorphic.</li>
     *         <li>Otherwise, it builds a rooted tree for each forest and checks if the two trees are isomorphic.</li>
     *         <li>If yes, it will construct a mapping for the forests from the created trees.</li>
     *     </ul>
     * </p>
     ...
     * <p>
     *     The method adds a new root vertex and connects it to the roots of the forest.
     *     The new root is the maximum vertex number + 1.
     * </p>
     * @param forest forest
    ...
    private Pair<Integer, Graph<?,?>> buildRootedTree(Graph<?,?> forest, Set<Integer> roots) {
        ...
        return new Pair<Integer, Graph<?,?>>(newRoot, Graphs.union(newForest, forest));
    }
}
