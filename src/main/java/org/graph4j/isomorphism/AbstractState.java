 ...
 * </p>
 * <p>
 * A partial mapping works only with indices of the vertices in the ordered
 * digraph(meaning the position in the sorted list of the vertices).
 * </p>
 *
...
public abstract class AbstractState implements State {
    ...
    public Isomorphism getMapping() {
        ...
        return new Isomorphism(o1.getGraph(), o2.getGraph(), forwardMap, backwardMap);
    }

    /**
     * If the graphs allow self loops, the vertices must have the same number of
     * self loops
     *
     * @param vertexIndex1 the index of a vertex from the ordered digraph 1 (the
     * index in the sorted list of vertices)
     * @param vertexIndex2 the index of a vertex from the ordered digraph 2 (the
     * index in the sorted list of vertices)
     *
    ...
    protected boolean compatibleVertices(int vertexIndex1, int vertexIndex2) {
        ...
        if (o1.getGraph() instanceof DirectedPseudograph
                && o2.getGraph() instanceof DirectedPseudograph) {
            DirectedPseudograph ps1 = (DirectedPseudograph) o1.getGraph();
            DirectedPseudograph ps2 = (DirectedPseudograph) o2.getGraph();
    ...
    /**
     * If the graphs allow multiple edges, the edges must have the same
     * multiplicity
     *
     * @param i1 the index of the source vertex of the edge in the ordered
     * digraph 1
     * @param j1 the index of the target vertex of the edge in the ordered
     * digraph 1
     *
     * @param i2 the index of the source vertex of the edge in the ordered
     * digraph 2
     * @param j2 the index of the target vertex of the edge in the ordered
     * digraph 2
     *
    ...
    protected boolean compatibleEdges(int i1, int j1, int i2, int j2) {
        ...
        if (o1.getGraph() instanceof DirectedMultigraph
                && o2.getGraph() instanceof DirectedMultigraph) {
            DirectedMultigraph mg1 = (DirectedMultigraph) o1.getGraph();
            DirectedMultigraph mg2 = (DirectedMultigraph) o2.getGraph();
