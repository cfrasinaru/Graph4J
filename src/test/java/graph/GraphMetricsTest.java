...
public class GraphMetricsTest {
    ...
    public void test1() {
        ...
        assertArrayEquals(new int[]{3, 2, 3, 3, 3, 4, 4}, gm.computeOrGetEccentricities());
        assertEquals(4, gm.diameter());
        assertEquals(2, gm.radius());
        assertEquals(Integer.MAX_VALUE, gm.girth());
        assertEquals(new VertexSet(g, 2), gm.center());
        assertEquals(new VertexSet(g, 6, 7), gm.periphery());
        assertEquals(new VertexSet(g, 6, 7), gm.pseudoPeriphery());
    }
    ...
    public void test2() {
        ...
        var gm = new GraphMetrics(g);
        assertArrayEquals(new int[]{4, 3, 4, 3, 4, 3, 4, 3, 3}, gm.computeOrGetEccentricities());
        assertEquals(4, gm.diameter());
        assertEquals(3, gm.radius());
        assertEquals(4, gm.girth());
        assertEquals(new VertexSet(g, 2, 4, 6, 8, 9), gm.center());
        assertEquals(new VertexSet(g, 1, 3, 5, 7), gm.periphery());
        assertEquals(new VertexSet(g, 1, 3, 4, 5, 7, 8, 9), gm.pseudoPeriphery());
    }
}
